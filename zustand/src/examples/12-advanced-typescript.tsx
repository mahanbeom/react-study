  ...
  return (
    <>
      ...
      <Notes
        ...
        questions={[
          {
            q: '설명을 들어도 커스텀 미들웨어 타이핑이 잘 이해가 안 간다. 실무에 필요한 내용인가? 더 파야 하나?',
            a: (
              <>
                <b>더 팔 필요 없다.</b> 실무 중요도로 나누면 — ① combine + replace 는 액션을
                지운다(함정, 기억할 것). ② devtools 는 가장 바깥(규칙만 따르면 됨). ③ &quot;미들웨어
                = initializer 를 감싸 <code>set</code> 을 바꿔치기하는 함수&quot; 라는 멘탈 모델 한
                문장(persist · devtools 가 <i>어떻게</i> 끼어드는지의 감). 여기까지가 가져갈 전부다.
                <br />
                <br />
                이중 타입(Logger/LoggerImpl) · Mps/Mcs · higher-kinded mutator 는{' '}
                <b>커스텀 미들웨어를 직접 만들 때만</b> 필요한데 실무에서 그럴 일이 거의 없다. 공식
                문서도 이 구현 세부를 본문이 아닌 GitHub 이슈(#710) 링크로 빼놓고 &quot;복잡한 타입
                문제를 다룰 때만 이해하면 된다&quot; 고 못 박는다. zustand <b>사용자</b>가 아니라{' '}
                <b>기여자 · 라이브러리 작성자</b>용 지식이다. 나중에 필요해지면 이 파일과 문서
                레시피를 <b>복사해다 고치면 된다</b> — 이 예제가 그 미래의 참조처다.
              </>
            ),
          },
        ]}
